...
for rsd_idx in "${!SEEDS[@]}"; do
    ...
    for src_idx in "${!TARGETS[@]}"; do
        ...
        case $source_tb in
            ar-padt)
            models=( bert-base-multilingual-cased xlm-roberta-base aubmindlab/bert-base-arabertv02 asafaya/bert-base-arabic )
            layers=( 6 6 6 6 )
            ;;

            en-ewt)
            models=( bert-base-multilingual-cased xlm-roberta-base bert-base-uncased roberta-large distilbert-bert-uncased )
            layers=( 6 6 6 12 3 )
            ;;

            fi-tdt)
            models=( bert-base-multilingual-cased xlm-roberta-base TurkuNLP/bert-base-finnish-uncased-v1 TurkuNLP/bert-base-finnish-cased-v1 )
            layers=( 6 6 6 6 )
            ;;

            grc-proiel)
            models=( bert-base-multilingual-cased xlm-roberta-base pranaydeeps/Ancient-Greek-BERT nlpaueb/bert-base-greek-uncased-v1 )
            layers=( 6 6 6 6 )
            ;;

            he-htb)
            models=( bert-base-multilingual-cased xlm-roberta-base onlplab/alephbert-base )
            layers=( 6 6 6 )
            ;;

            ko-gsd)
            models=( bert-base-multilingual-cased xlm-roberta-base klue/bert-base klue/roberta-large kykim/bert-kor-base )
            layers=( 6 6 6 12 6 )
            ;;

            ru-gsd)
            models=( bert-base-multilingual-cased xlm-roberta-base cointegrated/rubert-tiny sberbank-ai/ruRoberta-large blinoff/roberta-base-russian-v0 )
            layers=( 6 6 2 12 6 )
            ;;

            sv-talbanken)
            models=( bert-base-multilingual-cased xlm-roberta-base KB/bert-base-swedish-cased )
            layers=( 6 6 6 )
            ;;

            zh-gsd)
            models=( bert-base-multilingual-cased xlm-roberta-base bert-base-chinese hfl/chinese-bert-wwm-ext hfl/chinese-roberta-wwm-ext )
            layers=( 6 6 6 6 6 )
            ;;
        ...
        for mdl_idx in "${!models[@]}"; do
            ...
            python train.py ${tb_dir} ${exp_dir} \
                -lm ${models[$mdl_idx]} \
                -pt rooted -el ${layers[$mdl_idx]} ${layers[$mdl_idx]} \
                -e 30 -es 3 -rs ${SEEDS[$rsd_idx]} \
                -td
            echo "DepProbe trained on ${source_tb} using ${models[$mdl_idx]}." > ${exp_dir}/experiment-info.txt
            ((num_train++))
            ...
            python predict.py ${exp_dir} ${target_file} ${pred_file} \
                -pt rooted -lm ${models[$mdl_idx]} -el ${layers[$mdl_idx]} ${layers[$mdl_idx]}
